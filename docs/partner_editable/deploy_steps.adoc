...
. Make sure that your AWS account is configured correctly, as discussed in the <<_technical_requirements,Technical requirements>> section.
...
[cols=2*]
|===
^|https://fwd.aws/3xyNN[Deploy {partner-product-name} into a new VPC on AWS^]
^|https://fwd.aws/Bx7em[View template^]

^|https://fwd.aws/wxQDK[Deploy {partner-product-name} into an existing VPC on AWS^]
